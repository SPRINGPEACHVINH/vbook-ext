# Copilot Instructions for metruyenchucv

## Project Overview
This project is a JavaScript-based extension for vbook app, target to fetch and organize book data from original web page, organized under the `src/` directory. Each file in `src/` represents a distinct feature or service, such as book management, chapter handling, bookmarks, genres, ranking, and search. The codebase is modular, with each file typically exporting functions for a specific domain.

## Key Architectural Patterns
- **File-per-feature:** Each major feature (e.g., `book.js`, `chap.js`, `genre.js`) is implemented in its own file. Cross-feature logic is minimal; files are mostly independent.
- **Stateless modules:** Most modules export pure functions or stateless logic. Shared state is rare and should be handled explicitly.
- **Data flow:** Data is passed between modules via function arguments and return values, not via global state.
- **Configuration:** Sensitive or environment-specific data (e.g., API keys) is stored in `src/key.txt` and loaded as needed.

## Developer Workflows
- **No build step:** The project is plain JavaScript; no transpilation or bundling is required.
- **Testing:** No formal test suite is present. Manual testing is expected, typically by running the extension/plugin in its target environment.
- **Debugging:** Debug by adding `console.log` statements or using the debugger in the host environment.

## Project-Specific Conventions
- **Naming:** File names are short and descriptive (e.g., `chap.js` for chapters, `recents.js` for recent items).
- **Exports:** Each file exports its main functionality as module exports. Avoid default exports; use named exports for clarity.
- **Error handling:** Functions should return `null` or throw errors on failure, as appropriate for the context.
- **Encoding:** Use `base64.js` for encoding/decoding needs; do not reimplement base64 logic.
- **Crypto:** Use `crypto.js` for cryptographic operations; do not introduce new crypto dependencies.

## Integration Points
- **External APIs:** Some modules interact with external services (e.g., fetching book or chapter data). API endpoints and keys are managed in `config.js` and `key.txt`.
- **Cross-module usage:** Import only what you need from other modules. Avoid circular dependencies.
...
## Key Files
- `src/config.js`: Central configuration and API endpoints
- `src/key.txt`: API keys or sensitive data
- `src/base64.js`, `src/crypto.js`: Utility modules for encoding and cryptography
- `src/book.js`, `src/chap.js`, `src/genre.js`, etc.: Feature modules

## When in Doubt
- Follow the structure and patterns of existing files.
- Prefer composition over inheritance.
- Keep modules focused and independent.

## Output Expectations
- Code should be clean, modular, and consistent with existing patterns.
- Avoid introducing new dependencies unless absolutely necessary.
- List the changes made in the response, do not use emoji.
---
